...
UPDATED VERSION with Eye Detection Fix (Quick Fix #1)
Real-time video streaming with CNN + Eye Detection Hybrid Approach
"""
...
class SystemState:
    def __init__(self):
        ...
        self.use_eye_detection = True  # Enable eye detection fix by default
...
def detect_eyes_in_face(face_roi):
    """
    Detect eyes in face region using Haar Cascade
...
def estimate_drowsiness_from_eyes(num_eyes):
    """
    Quick drowsiness estimation based on eyes detected
    
    Logic:
    - 2 eyes detected: Eyes open → Low drowsiness
    - 1 eye detected: One eye closing → Medium drowsiness
    - 0 eyes detected: Eyes closed → High drowsiness
    
    Add small random variation to avoid constant same score
    """
    if num_eyes >= 2:
        # Both eyes visible = Alert
        base_drowsiness = 0.2
        base_cognitive = 0.1
    elif num_eyes == 1:
        # One eye visible = Partially drowsy
        base_drowsiness = 0.55
        base_cognitive = 0.45
    else:
        # No eyes visible = Drowsy
        base_drowsiness = 0.8
        base_cognitive = 0.7
    
    # Add small random noise for variation
    drowsiness_score = base_drowsiness + np.random.normal(0, 0.05)
    cognitive_fatigue = base_cognitive + np.random.normal(0, 0.05)
    
    # Clip to [0, 1]
    drowsiness_score = np.clip(drowsiness_score, 0, 1)
    cognitive_fatigue = np.clip(cognitive_fatigue, 0, 1)
    
    return drowsiness_score, cognitive_fatigue
...
class VideoProcessor:
    ...
    def process_frame(self, frame):
        ...
        1. Detect & crop face
        2. Preprocess
        3. Extract features (Eye Detection OR CNN)
        ...
        # ===== UPDATED: USE EYE DETECTION FIX =====
        if state.use_eye_detection:
            # Quick Fix #1: Eye Detection-based drowsiness estimation
            num_eyes = detect_eyes_in_face(face_roi)
            drowsiness_score, cognitive_fatigue = estimate_drowsiness_from_eyes(num_eyes)
            
            logger.debug(f"Eyes detected: {num_eyes} → Drowsiness: {drowsiness_score:.3f}")
        else:
            # Original: CNN-based (random model)
        ...
        # Adaptive Risk Fusion (More sensitive version)
        ...
        method = "Eye Detection" if state.use_eye_detection else "CNN Model"
        cv2.putText(frame, f"Method: {method}", (10, info_y+100),
    ...
    def adaptive_fusion(drowsiness, cognitive_fatigue, w1=0.6, w2=0.4):
        ...
        Using exponential weighting to amplify drowsiness signals
        """
        # Original weighted average
        # fused = (w1 * drowsiness) + (w2 * cognitive_fatigue)
        
        # UPDATED: More sensitive exponential fusion
        # This amplifies higher scores while keeping low scores low
        fused = (w1 * (drowsiness ** 1.3)) + (w2 * (cognitive_fatigue ** 1.3))
        
        # Alternative: Maximum of both (even more sensitive)
        # fused = max(drowsiness, cognitive_fatigue)
...
def get_metrics():
    ...
    return jsonify({
        ...
        'detection_method': 'Eye Detection' if state.use_eye_detection else 'CNN Model'
    })
...
@app.route('/api/toggle_method')
def toggle_detection_method():
    """Toggle between eye detection and CNN model"""
    state.use_eye_detection = not state.use_eye_detection
    method = 'Eye Detection' if state.use_eye_detection else 'CNN Model'
    logger.info(f"Switched to: {method}")
    return jsonify({
        'status': 'method_switched',
        'current_method': method,
        'use_eye_detection': state.use_eye_detection
    })
...
def initialize_system():
    ...
    try:
        ...
        logger.info(f"Detection method: {'Eye Detection (Fix #1)' if state.use_eye_detection else 'CNN Model'}")
...
if __name__ == '__main__':
    ...
    logger.info("="*60)
    logger.info("Detection Method: Eye Detection (Quick Fix #1)")
    logger.info("✓ Close eyes → Score goes HIGH")
    logger.info("✓ Open eyes → Score goes LOW")
    logger.info("✓ Ready for real drowsiness detection!")
    logger.info("="*60)
